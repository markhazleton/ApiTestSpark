...
bold_scripts_dir="$repo_root/.bold/scripts"
bold_scripts_present=false
[ -d "$bold_scripts_dir" ] && bold_scripts_present=true
# Per carried script: would three-tier resolution ever serve it? An override
# is live only at an exact relative-path match against .bold/scripts/
# (bold-which's rule), so this is measurable -- the prompt must never
# re-derive it (issue #2: 15 dead scripts sat unflagged in a migrated repo).
team_script_entries=()
if [ -d "$team_scripts_dir" ]; then
  has_team_scripts_override=true
  while IFS= read -r f; do
    [ -n "$f" ] || continue
    rel="${f#"$team_scripts_dir"/}"
    resolves=false
    [ -f "$bold_scripts_dir/$rel" ] && resolves=true
    team_script_entries+=("{\"path\":\"$(json_escape "$rel")\",\"resolves_against_bold\":$resolves}")
  done < <(find "$team_scripts_dir" -type f | LC_ALL=C sort)
fi
team_scripts_json="[$(IFS=,; echo "${team_script_entries[*]}")]"
[ "${#team_script_entries[@]}" -eq 0 ] && team_scripts_json="[]"
...
# Verification sweep: every tracked file (git ls-files, fast, respects
# .gitignore for untracked noise) mentioning "devspark", excluding .archive/
# (legitimate historical references belong there). Sweeps every tracked file,
# not an extension allowlist -- a real repo's stale DevSpark reference can
# live in an extensionless file (.gitignore, Dockerfile, .editorconfig) as
# easily as a .md one, and an allowlist only sees what it was told to expect.
# grep -I skips binary files automatically (no separate check needed).
devspark_ref_files=()
if [ -e "$repo_root/.git" ]; then
  while IFS= read -r f; do
    ...
    full="$repo_root/$f"
    [ -f "$full" ] || continue
    if grep -qilI "devspark" "$full" 2>/dev/null; then
...
printf '{"has_devspark_dir":%s,"devspark_dir_file_count":%s,"has_documentation_dir":%s,"has_constitution":%s,"constitution_principle_count":%s,"has_team_scripts_override":%s,"bold_scripts_present":%s,"team_scripts":%s,"spec_dirs":%s,"documentation_other_entries":%s,"has_existing_archive":%s,"existing_archive_entries":%s,"legacy_claude_commands":%s,"legacy_github_agents":%s,"legacy_github_prompts":%s,"has_agents_registry":%s,"has_multi_app_config":%s,"devspark_reference_files":%s,"user":"%s"}' \
  ...
  "$has_agents_registry" "$has_multi_app_config" "$devspark_ref_json" "$(json_escape "$user_slug")"
