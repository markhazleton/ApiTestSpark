...
Run the collector. It hard-fails — nonzero exit, no JSON — in two cases, unconditionally before anything else runs. First, if the branch is behind `base_branch` (`git_status.proceed` would read `false`): a build spends a full cycle of work on top of whatever's checked out, and starting or continuing that cycle on a base that's already moved on origin is how a feature ends up needing a large, late reconciliation merge instead of a small early one (see `bold-docs/patches.md`, 2026-07-18). Second, on a Feature-tier feature (matched by the current branch name), if `gates/analyze.md`, `gates/critic.md`, or `gates/checklist.md` are missing or still have an open finding/unchecked item (see `bold-docs/patches.md`, 2026-07-16). Either way this is a real gate, not advisory text: if the collector exits nonzero, stop immediately, show the human the exact failure, and route to `bold.plan` (to sync/reconcile) or `bold.plan analyze` / `bold.plan critic` / `bold.plan checklist` as appropriate. **Never write the gate files yourself, or merge/rebase blindly, just to get past the check** — a gate file authored after the fact reviewed nothing, and a blind sync can silently overwrite work in progress.

Once the collector succeeds, read its output. Find the active feature's ratified tier in `active_features`. If none is ratified, halt and route to `bold.plan` — build never guesses a tier.
