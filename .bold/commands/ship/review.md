...
The collector itself enforces this — nonzero exit, no JSON, before this subcommand ever gets a chance to review anything — if the branch is behind `base_branch`. A review against a stale base is reviewing the wrong diff. If you somehow reach this point with `git_status.commits_behind` greater than 0 anyway, treat it as a bug in the check, not permission to proceed: stop and say so using `git_status.message`, same as `bold.ship` (default) does.
