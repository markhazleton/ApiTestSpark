...
4. Pick which script to run: read `.bold-user/{git-user-name}/preferences.json`'s `shell` field, if that file already exists (an upgrade on a repo where `bold.personalize` has been used before). Otherwise — including every fresh install, since the file can't exist before this step scaffolds `.bold-user/` — fall back to the same OS-detected default `bold.personalize` itself uses: PowerShell on Windows, bash elsewhere. Run the matching script — `install.ps1 -Source {source} -Target {target}` for PowerShell, `install.sh --source {source} --target {target}` for bash — deterministically, never "or the other one" left to guesswork. This is the only step that executes anything, and only after step 2's approval.
